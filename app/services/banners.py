...
class BannerService:
            ...
            with urllib.request.urlopen(req, timeout=5) as resp:
                if resp.status == 200:
                    ...
                    for g in games:
                        ...
                        # Artworks in 1080p (max 2 per game)
                        for art in g.get("artworks", [])[:2]:
                        ...
                        # Screenshots in 1080p (max 2 per game)
                        for sc in g.get("screenshots", [])[:2]:
    ...
    def _calculate_relevance(cls, item: BannerSearchResult, clean_query: str, raw_query: str) -> int:
        ...
        # 1. Exact match on work title or origin
        if origin_norm == query_norm_clean or origin_norm == query_norm_raw:
            return 5000
        if name_norm.startswith(query_norm_clean) or name_norm.startswith(query_norm_raw):
            return 4500
        if origin_norm.startswith(query_norm_clean) or origin_norm.startswith(query_norm_raw):
            return 4300
