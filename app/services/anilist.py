...
class AnilistService:
    ...
    def get_trending_manga() -> List[SearchResultItem]:
        # Implementation using sort: TRENDING_DESC
        url = "https://graphql.anilist.co"
        graphql_query = '''
        query {
            ...
            with urllib.request.urlopen(req, timeout=5) as response:
                if response.status == 200:
                    ...
                    for item in media_list:
                        ...
                        desc = item.get("description") or ""
                        desc = re.sub('<[^<]+?>', '', desc)
                        results.append(SearchResultItem(external_id=str(item.get("id")), title=title, image_url=image_url, description=desc, item_type="manga", release_date=release_date, popularity=float(item.get("averageScore") or 0)))
                    return results
        except Exception:
            pass
        return []
